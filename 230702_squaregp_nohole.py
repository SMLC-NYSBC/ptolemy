...
while sum(requester.get_current_lm_state().visited) < 462:
    ...
    for sq_img_id, sq_img in square_images.items():
        ...
        for hole_id, row in res.iterrows():
            # if row.prior_score < 0.2 #  or row.ctf_pred > 7:
            #     continue
...
requester.save_state('/h2/pkim/ptolemy/m23feb16a_results/230703_nohole_correctnorm_terminalstate')
